...
const loadCategories = async () => {
  ...
  const displayCategories = (data) => {
    ...
    data.forEach((category, index) => {
      const div = document.createElement("div");
      div.innerHTML = `
        <button id="${category.category_id}" onclick="handleCategoryClick(${index}, ${category.category_id})" class="btn ${index === 0 ? "bg-[#ff1f3d] text-white" : ""}">
          ${category?.category}
        </button>`;
      allCategory.appendChild(div);
    });
...
const handleSort = () => {
    ...
    const orderText = isAscending ? "Lowest to Highest" : "Highest to Lowest";
    sortContainer.setAttribute("data-tip", orderText);
  
    videos.sort((a, b) =>
      (parseInt(a.others.views) || 0) - (parseInt(b.others.views) || 0) * (isAscending ? 1 : -1)
    );
